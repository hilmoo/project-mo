...
export default function InfoLomba() {
  const { categoryArray, competition } = useLoaderData<typeof loader>();
  ...
  const [filteredData, setFilteredData] = useState<CompetitionSimple[]>([]);
  useEffect(() => {
  ...
  return (
    <>
      ...
      <Container size="xxl" maw={2000}>
        <Grid>
          <Await resolve={categoryArray}>{filterFull}</Await>
          <Grid.Col span="auto">
            <SimpleGrid
